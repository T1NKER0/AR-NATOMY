...
public class applicationDBAuthenticationGoodComplete{
	...
	*/
	...
	// Add this method to the applicationDBAuthenticationGoodComplete class
public ResultSet showTables() {
    ...
    try {
        // Create a statement using the connection from myDBConn
        statement = myDBConn.getConnection().createStatement();
	...
	*/
	...
	public ResultSet getUserGrades(String userName) {

		// Declare and define fields to retrieve from the grades table
		String fields = "*"; // You can specify particular columns like "assessment_id, grade" if needed
	
		// Declare and define the table where selection is performed
		String table = "assessments"; // Adjust the table name if necessary
	
		// Add a WHERE clause to filter grades by the logged-in user
		String condition = "userstudent = '" + userName + "'"; // Assuming grades are associated with the username
	
		// Return the ResultSet containing grades for the logged-in user
		return myDBConn.doSelect(fields, table, condition);
	}
	
	/*******
    modifyGrade method
        Modifies an existing grade in the database for a specific user and assessment
        @parameters:
            assessmentId - the ID of the assessment
            userName - the username of the student whose grade is being modified
            newGrade - the new grade to be set
        @returns:
            A boolean indicating if the grade has been successfully modified.
*/
public boolean modifyGrade(int assessmentId, String userName, double newGrade) {

    // Define table name and fields to update
    String table = "assessments";
    String setClause = "grade = " + newGrade; // Update the grade field

    // Define the WHERE clause to identify the specific grade record
    String whereClause = "assessment_id = " + assessmentId + " AND userstudent = '" + userName + "'";

    // Use the doUpdate method to perform the update
    boolean res = myDBConn.doUpdate(table, setClause, whereClause);

    if (res) {
        System.out.println("Grade updated successfully for user: " + userName + ", assessment ID: " + assessmentId);
    } else {
        System.out.println("Failed to update grade for user: " + userName + ", assessment ID: " + assessmentId);
    }
...
public ResultSet getAssessmentVisibility(int assessmentId) {
    // Declare the table and the fields you want to retrieve
    String fields = "isVisible";
    String table = "assessment_types";

    // Define the condition to select the specific assessment by ID
    String whereClause = "assessment_type_id = " + assessmentId;

    // Return the ResultSet containing the visibility status of the assessment
    return myDBConn.doSelect(fields, table, whereClause);
}


public boolean updateAssessmentVisibility(int assessmentId, boolean isVisible) {
    boolean res = false;
    ...
    try {
        ...
        // Convert the boolean visibility value to 1 (true) or 0 (false)
        int visibilityValue = isVisible ? 1 : 0;

        // Set the clause for updating the visibility
        String setClause = "isVisible = " + visibilityValue;
        
        // Define the where clause to identify the assessment by ID
        String whereClause = "assessment_type_id = " + assessmentId;
        ...
        if (res) {
            System.out.println("Assessment visibility updated successfully for assessment ID: " + assessmentId);
        } else {
            System.out.println("No assessment found with ID: " + assessmentId);
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
...
/*******
    getAssessmentDetails method
        Retrieves the assessment and visibility status for a specific assessment ID.
        @parameters:
            assessmentId - the ID of the assessment
        @returns:
            A ResultSet containing the assessment information and its visibility status.
*/
public ResultSet getAssessmentDetails(int assessmentId) {
    // Declare the fields to retrieve
    String fields = "name, description, isVisible"; // Fields based on your table's structure
    String table = "assessment_types"; // Table name
    
    // Define the condition to select the specific assessment by ID
    String whereClause = "assessment_type_id = " + assessmentId; // Correct column for ID is assessment_type_id

    // Return the ResultSet containing the assessment details and visibility status
    return myDBConn.doSelect(fields, table, whereClause);
}
	...
	*/
	...
	public ResultSet getAssessments() {
		ResultSet resultSet = null;
		Statement statement = null;
	
		// Create the SQL query to retrieve grades
		String query = "SELECT * FROM assessments"; // This query can be customized as needed
